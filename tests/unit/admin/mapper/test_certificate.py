...
def test_certificate_to_response(optional_is_none: bool) -> None:
    ...
    assert isinstance(mdl, schema.CertificateResponse)

    asserts_generator.assert_class_instance_equality(schema.CertificateResponse, cert, mdl)
...
def test_aggregator_to_page_response() -> None:
    ...
    assert isinstance(mdl, schema.CertificatePageResponse)
    assert len(mdl.certificates) == 2
    assert all([isinstance(a, schema.CertificateResponse) for a in mdl.certificates])
