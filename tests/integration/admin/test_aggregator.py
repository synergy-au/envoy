...
async def test_get_all_aggregators(
    ...
    res = await admin_client_auth.get(schema.uri.AggregatorListUri + _build_query_string(start, limit))
    assert res.status_code == HTTPStatus.OK

    body = response.read_response_body_string(res)
    assert len(body) > 0
    agg_page: schema.AggregatorPageResponse = schema.AggregatorPageResponse(**json.loads(body))
    ...
    assert all([isinstance(s, schema.AggregatorResponse) for s in agg_page.aggregators])
...
async def test_get_aggregator(
    ...
    res = await admin_client_auth.get(schema.uri.AggregatorUri.format(aggregator_id=agg_id))
    if expected_name is None or expected_domains is None:
        ...
        body = response.read_response_body_string(res)
        assert len(body) > 0
        agg: schema.AggregatorResponse = schema.AggregatorResponse(**json.loads(body))

        assert agg.aggregator_id == agg_id
        assert agg.name == expected_name
        assert all([isinstance(s, schema.AggregatorDomain) for s in agg.domains])
...
async def test_get_aggregator_certificates(
    ...
    res = await admin_client_auth.get(
        schema.uri.AggregatorCertificateListUri.format(aggregator_id=agg_id) + _build_query_string(start, limit)
    )
    ...
    body = response.read_response_body_string(res)
    assert len(body) > 0
    cert_page: schema.CertificatePageResponse = schema.CertificatePageResponse.model_validate_json(body)
    ...
    assert all([isinstance(s, schema.CertificateResponse) for s in cert_page.certificates])
